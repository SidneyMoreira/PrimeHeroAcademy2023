...
A partir da lista ["A", 1, "B", 2, "C" e 3], adicionar somente os números numa nova lista
    ...
    FOR  ${ITEM}  IN  @{LISTA_MISTA}
        TRY
            Evaluate  ${ITEM} + 1
            Append to list  ${NUMEROS}  ${ITEM}
            Log to console  \nAdicionei o elemento ${ITEM} a lista pq é um número
        EXCEPT
            Log to console  Nao adicionei o elemento ${ITEM} a lista pq nao é um número
        FINALLY
            Log to console  Lista atualizada nesta iteração: @{NUMEROS}
            Log to console  \n
        END
    END